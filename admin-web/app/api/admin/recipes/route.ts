...
import { deleteRecipe, fetchRecipesForMealType, upsertRecipe } from '@/lib/firestore-repository';
...
export async function GET(request: Request) {
  try {
    ...
    const includeInactive = (url.searchParams.get('includeInactive') ?? '').trim() === '1';
    const items = await fetchRecipesForMealType(mealTypeId, 500, !includeInactive);
